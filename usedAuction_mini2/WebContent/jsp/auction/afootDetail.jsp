...
<div class="row">
	<div class="col-md-10">
		...
		<!-- Modal -->
		<div id="doAuctionModal" class="modal fade" role="dialog">
				...
				<div class="modal-content">
					...
					<div class="modal-body">
						<div class="form-group">
							<label for="usr">현재가:</label> <input type="text"
								class="form-control" id="usr" readonly value="${currentPrice }">
						</div>
						<form name="bidForm" action="insertBid.do" method="post">
						<input type="hidden" name="acNo" value="${acNo }"/>
						<div class="form-group">
							<label for="bidPrice">입찰가:</label> <input type="text"
								name="bidPrice"	class="form-control" id="bidPrice">
						</div>
						</form>
					</div>
					<div class="modal-footer">
						<button type="button" onclick="insertBid();" class="btn btn-primary" data-dismiss="modal">등록</button>
		...
		<table>
			...
			<td colspan="3" align="right">
				...
				<c:when test="${not empty user}">
					<button type="button" class="btn btn-info btn-lg"
						data-toggle="modal" data-target="#doAuctionModal">입찰하기</button>
				</c:when>
				<c:otherwise>
					<button type="button" class="btn btn-info btn-lg"
						onclick="swal('입찰오류','로그인 하셔야 입찰하실 수 있습니다.', 'error');">입찰하기</button>
			...
			<tr>
				<td>시작가격</td>
				<td><input type="text" readonly value="${auction.acStartPrice }" />원</td>
			...
			<tr>
				...
				<input type="hidden" name="page" value="afoot"/>
...
<script>
	var result = "${result}";
	if(result === 'ok') {
		swal('입찰결과', '입찰하였습니다.', 'success');
	}else if(result === 'fail') {
		swal('입찰오류','입찰에 실패하였습니다.','error');
	}
	
	
	function insertBid() {
		var frm = document.bidForm;
		var bidPrice = document.querySelector("#bidPrice");
		if(!bidPrice.value) {
			swal('입찰오류','입찰시 입찰가를 입력해 주세요','error');
			return;
		}
		frm.submit();
	}
